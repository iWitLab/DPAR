...
class Authenticate:
    ...
    def register_user(self, form_name: str, location: str='main', preauthorization=False) -> bool:
        ...
        register_user_form.subheader("Registration From")
        ...
        if register_user_form.form_submit_button('Register'):
            ### Form validation
            if len(new_email) * len(new_username) * len(new_password) == 0:
                form_error = True
                form_message = 'Please complete the registration form'
            if new_password != new_password_repeat:
                form_error = True
                form_message = 'Mismatch between entered passwords!'
            if new_username in self.credentials['usernames']:
                form_error = True
                form_message = 'Username already taken, please choose a new username'
            if len(new_password) < 8:
                form_error = True
                form_message = 'Your password must contain at least 8 characters'

            if form_error:
                raise RegisterError(form_message)
            else:
                self._register_credentials(new_username, new_password, new_email, preauthorization)
                registered_flag = True
                return_values = [registered_flag, new_username, new_password, new_email]
